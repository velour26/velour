...
- `customer` — покупатель (публичный сайт);
- `employee` — сотрудник магазина (делает заказы для магазина);
- `manager` — менеджер (управляет складом, видит заказы от магазинов);
- `admin` — администратор (управляет магазинами, прикрепляет сотрудников).

Модель `Store`:
- хранит розничные магазины;
- поле `manager` (FK на менеджера) и `employees` (M2M на сотрудников);
- управляется администратором через Django admin.
...
- `employee1@velour.ru` ... `employee3@velour.ru` с паролем `Employee123!`;
- `user1@example.com` ... `user20@example.com` с паролем `User123!`;
- 2 магазина с привязкой сотрудников и менеджеров;
- 1 склад с демо-приёмкой и ревизией;
